...
function sendemail($mail_username,$mail_userpassword,$mail_setFromEmail,$mail_setFromName,$mail_addAddress,$txt_message,$mail_subject, $template){
    ...
    $mail->SMTPSecure = 'tls';                  // Habilitar encriptacion, `tls` es aceptada
    $mail->Port = 587;                          // Puerto TCP  para conectarse 
    $mail->setFrom($mail_username, "Corriente Fluye");//Introduzca la dirección de la que debe aparecer el correo electrónico. Puede utilizar cualquier dirección que el servidor SMTP acepte como válida. El segundo parámetro opcional para esta función es el nombre que se mostrará como el remitente en lugar  de la dirección de correo electrónico en sí.
    ...
    if(!$mail->send()) {
        echo '<p style="color:red">No se pudo enviar..';
        echo 'Error de correo: ' . $mail->ErrorInfo."</p>";
    }  else {
        echo '<p style="color:green">Ha sido enviado!</p>';
    }
}
?>